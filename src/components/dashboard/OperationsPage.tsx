...
const recentActivities: Activity[] = [
  {
    id: 'ACT-001',
    type: 'success',
    message: 'Shipment #999 completed and dispatched',
    time: '10 minutes ago'
  },
  {
    id: 'ACT-002',
    type: 'warning',
    message: 'Inventory alert: Item #42 running low (12 units left)',
    time: '25 minutes ago'
  },
  {
    id: 'ACT-003',
    type: 'info',
    message: 'New order received from Acme Corp ($45,000)',
    time: '1 hour ago'
  },
  {
    id: 'ACT-004',
    type: 'success',
    message: 'Quality check passed for Production #909',
    time: '2 hours ago'
  },
  {
    id: 'ACT-005',
    type: 'error',
    message: 'Machine B maintenance overdue by 2 days',
    time: '3 hours ago'
  }
]
...
function PriorityBadge({ priority }: { priority: Workflow['priority'] }) {
  const config = {
    low: { className: 'bg-slate-100 text-slate-500', dot: 'bg-slate-400' },
    medium: { className: 'bg-blue-100 text-blue-600', dot: 'bg-blue-500' },
    high: { className: 'bg-orange-100 text-orange-600', dot: 'bg-orange-500' },
    urgent: { className: 'bg-red-100 text-red-600', dot: 'bg-red-500 animate-pulse' }
  }

  const { className, dot } = config[priority]
  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl border border-white/40 rounded-xl p-5 hover:-translate-y-0.5 transition-all duration-300 shadow-lg shadow-indigo-500/4 group">
      ...
      <div className="flex items-center justify-between mt-4 pt-3 border-t border-slate-100">
        <span className="text-xs text-slate-400">Due: {workflow.dueDate}</span>
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-base md:text-lg text-slate-500">
            Monitor production, orders, and maintenance workflows.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-600"><strong>{workflows.filter(w => w.status === 'in-progress').length}</strong> Active</span>
          </div>
          <div className="flex items-center gap-2">
            <CheckCircle2 size={16} className="text-green-500" />
            <span className="text-slate-600"><strong>{workflows.filter(w => w.status === 'completed').length}</strong> Done</span>
          </div>
          <div className="flex items-center gap-2">
            <Clock size={16} className="text-slate-400" />
            <span className="text-slate-600"><strong>{workflows.filter(w => w.status === 'pending').length}</strong> Pending</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {operationsKPIs.map((kpi) => (
          <OpsKPICard key={kpi.title} kpi={kpi} />
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-violet-600 rounded-xl p-5 text-white shadow-xl">
            ...
            <p className="text-sm text-indigo-100 mb-4">Common operations tasks</p>
